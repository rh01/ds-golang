...
func IntCallbackIterator(cb func(int)) {
	for _, n := range int_data {
		cb(n)
	}
}

func DataCallbackIterator(cb func(*Data)) {
	for _, n := range struct_data {
		cb(n)
	}
}
...
// func NewDataStatefulIteratorInterface(data []*Data) *dataStatefulIterator {
